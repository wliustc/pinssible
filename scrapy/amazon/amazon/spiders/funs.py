...
def get_mysql_connection(db_user, db_pass, db):
	conn = MySQLdb.connect(host='localhost',user=db_user, passwd=db_pass, port=3306)	
	conn.select_db(db)
	cursor = conn.cursor()	
...
def get_item_detail(contents):
	...
	if (not product_description):
		product_description_array = soup.select("#productDescription > div.content > div")
		if (product_description_array):
			product_description = html_trim(product_description_array[0].get_text())
